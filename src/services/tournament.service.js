...
const { createMatchId } = require("../utils/sequence.util"); 
...
async function manualGroupAndGenerate(body) {
  ...
  for (const g of groups) {
    ...
    for (const R of rounds) {
      for (const p of R.pairs) {
        ...
        const matchId = createGroupMatchId(handLevel, g.letter, R.roundNo, order, 2);
  ...
  return {
    level: handLevel,
    groups: groups.map((g) => ({ letter: g.letter, teamCount: g.teamIds.length })),
