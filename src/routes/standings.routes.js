...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!handLevel)
        return res.status(400).json({ message: "handLevel is required" });
      ...
      // 3. Reset Stats
      // หมายเหตุ: การคำนวณใหม่ (Recalculate) ปกติเราจะไม่ล้าง couponsUsed เพราะถือว่าเป็นข้อมูลสะสมจริง
      // แต่จะล้างคะแนนเพื่อคำนวณจาก Match History ใหม่
...
router.post("/clear", authMiddleware, requireAdmin, async (req, res) => {
  try {
    ...
    // ✅ [แก้ไขจุดที่ 1] ล้างสถิติทีม + ล้างคูปองที่ใช้ (couponsUsed)
    await Team.updateMany(filter, {
      $set: {
        matchesPlayed: 0,
        wins: 0,
        ...
        setsDiff: 0,
        matchScores: [],
        couponsUsed: 0, // <--- เพิ่มตรงนี้ เพื่อล้างยอดคูปองด้วย
      },
    });

    if (resetMatches) {
      // ✅ [แก้ไขจุดที่ 2] ล้างข้อมูลแมตช์ + ล้างยอดลูกแบดในแมตช์ (shuttlecockUsed)
      await Match.updateMany(filter, {
        $set: {
          ...
          set2Score1: 0,
          set2Score2: 0,
          shuttlecockUsed: 0, // <--- เพิ่มตรงนี้ เพื่อล้างยอดลูกแบดในแมตช์
        },
      });
    }

    res.json({ message: "Cleared standings, matches, and coupons." });
  } catch (e) {
